...
const ensureDirectoryExistsAndSave = (filePath, data) => {
  try {
    ...
    console.error(`Error creating directories or saving file for ${filePath}: ${error.message}`);
  }
};

// Process activities for a specific day
const processActivitiesForDay = async (city, inquiry, formattedDate, travelers, inquiryToken, cityActivitiesTracker) => {
  try {
    const destinationData = await ActivityDestination.findOne({ destination_id: city.destination_id });
    ...
    console.error(`Error processing activities for ${city.city} on ${formattedDate}:`, error);
    return {
      date: formattedDate,
      activities: [],
      error: error.message
...
exports.createItinerary = async (req, res) => {
  try {
    ...
    const inquiry = await ItineraryInquiry.findOne({ itineraryInquiryToken: inquiryToken });
    ...
    for (const [index, { city, startDate, endDate }] of cityDayDistribution.entries()) {
      ...
      for (let dayOffset = 0; dayOffset < daysForThisCity; dayOffset++) {
        ...
        if (index === 0 && dayOffset === 0) {
          ...
          try {
            const arrivalLocation = formatLocationForTransfer({
              ...
              latitude: departureFlights[0].arrivalAirport?.location?.latitude || 
                       departureFlights[0].destination_location?.latitude,
              longitude: departureFlights[0].arrivalAirport?.location?.longitude || 
            ...
            const airportToHotelTransfer = await getGroundTransfer(transferRequestData);
        ...
        if (index === cityDayDistribution.length - 1 && dayOffset === daysForThisCity - 1) {
          dayObject.flights.push({
          ...
          try {
            ...
            const departureLocation = formatLocationForTransfer({
              ...
              latitude: returnFlights[0].originAirport?.location?.latitude || 
                       returnFlights[0].origin_location?.latitude,
              longitude: returnFlights[0].originAirport?.location?.longitude || 
            ...
            const hotelToAirportTransfer = await getGroundTransfer(transferRequestData);
    ...
    // Save debug file
    const debugFilePath = path.join(__dirname, "../../logs/itineraries", `${itineraryToken}.json`);
